...
def get_model() -> Tuple[Any, Optional[str]]:
    ...
    if not MODEL_PATH.exists():
        logger.warning("Modelo no encontrado en %s", path)
        return None, None
...
def predict_triage_ml(form: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        raw_pred = str(model.predict(X)[0]).strip().lower()
        pred = LABEL_MAP.get(raw_pred)

        proba = None
        if hasattr(model, "predict_proba"):
            probs = model.predict_proba(X)[0]
            raw_classes = [str(c).strip().lower() for c in getattr(model, "classes_", [])]
            if raw_pred in raw_classes:
                idx = raw_classes.index(raw_pred)
        ...
        logger.warning("Error en predict ML: %s", e)
        return {
            "ok": False,
            "pred": None,
            "proba": None,
...
def enriquecer_registro(r: dict) -> dict:
    ...
    color = (
        r.get("manchester", {}).get("color_final")
        or r.get("manchester_color_final")
        or "VERDE"
    ).upper()
...
async def triage_form(request: Request):
    if templates is None:
        return HTMLResponse("<h1>Hospital Triage API</h1><p>Templates no disponibles.</p>")
...
async def predict_json(payload: Dict[str, Any] = Body(...)):
    ...
    return {
        ...
        "demo": {
            "score": score,
            "probabilidad": prob_demo,
            "sugerido": sugerido_demo,
        },
    }
...
async def medico(request: Request):
    ...
    if templates is None:
        return JSONResponse(abiertos_sorted)
...
@app.post("/predict")
def predict_for_tests(payload: PredictRequest):
    ...
    try:
        raw_pred = str(model.predict(X)[0]).strip().lower()
        risk_level = raw_pred if raw_pred in {"low", "medium", "high"} else None
